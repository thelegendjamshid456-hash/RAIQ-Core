...
The repository now includes the first RAIQ Agent planning foundation: transparent technical-domain routing, adaptive plan depth, structured plans, tool-intent proposals, and append-only planning traces. It does not execute files, code, shell commands, search, retrieval, or network actions; see `docs/agent_foundation.md`. Every model-related push is evidence-gated: run `PYTHONPATH=. python3 scripts/validate_model_evidence.py --checkpoint <path>` and review `docs/evidence_gates.md` before claiming the model is functional or capable. Future work will add RAIQ Code, RAIQ Neural, RAIQ Chem, RAIQ Tools, RAIQ Memory, RAIQ RAG, and RAIQ Verify around the validated core. Any supplied policy or prompt material will be treated as a versioned project artifact for later agent-layer review; it does not replace tested model behavior, tool safety controls, or verification.
